...
import RequestBlock from "../components/RequestBlock.tsx";
import FleetSlider from "../components/FleetSlider.tsx";
...
export default function BusDetail() {
    ...
    const fetchBus = async () => {
        ...
        if (error) console.error(error);
            ...
            const foundBus = (data as Bus[]).find(
        ...
        <>
            ...
            <div className=" max-w-[1400px] w-[90%] m-auto  mx-auto mb-10 mt-28 md:mt-40">
                <div className="flex flex-col lg:flex-row gap-8 md:gap-12">
                             ...
                             rounded-full shadow-md backdrop-blur-sm"
                                ...
                                <button
                                    onClick={() => {
                                        const idx = bus.image_url!.indexOf(mainImage!);
                                        const next =
                                            idx === bus.image_url!.length - 1 ? 0 : idx + 1;
                  ...
                  Кол-во мест: {bus.seats}
                </span>
                            </div>
                            <div className="flex items-center gap-3">
                                <img src="/money.svg" className="w-8 h-8 text-blue-500" />
                                <span className="text-black text-2xl">
                  Стоимость: от {bus.price} руб./час
                </span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6 text-xl">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-black text-2xl">
                  Опыт водителя: {bus.driver_exp} лет
                </span>
                            </div>
                            <div className="flex items-center gap-3">
                                <Clock className="w-8 h-8 text-blue-500" />
                                <span className="text-black text-2xl">
                  Мин. время заказа: {bus.min_time} ч
                </span>
                            </div>
                        </div>
                    </div>
            ...
            <RequestBlock />
            <FleetSlider currentBusId={bus?.id || 0} />
